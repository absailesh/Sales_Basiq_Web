...
export class AddInfluencerComponent implements OnInit {
    ...
    constructor(
        ...
        InfluencerDetail(){
            this.service.post_rqst({'id':this.params_id},'Influencer/influencerCustomerDetail').subscribe((resp)=>
            {
                if (resp['statusCode'] == 200) {
                    ...
                    this.user_assign_detail = this.data['user_assign_detail']
                    this.data.user_assigned_id = this.data['user_assign_detail'][0]['user_id']+''
                    
                    if(this.checkRight.scanning_rights == 'No' && this.checkRight.point_transfer_right == 'No'){
                        this.data.architect_assign =  this.data['architect_detail']['assigned_influencer_id'];
                        this.data.contractor_assign = this.data['contractor_detail']['assigned_influencer_id'];

                        if(this.data.architect_assign){
                            this.architectList('');
                        }
                        if(this.data.contractor_assign){
                            this.contractortList('');
                        }
                    }
        ...
        architectList(searcValue) {
            this.service.post_rqst({'search':searcValue, 'point_transfer_right':'Yes', 'scanning_rights':'No'}, "Influencer/influencerAsPerScanningRights").subscribe((result => {
                if (result['statusCode'] == 200) {
                    this.architectUser = result['result'];
                }
                else{
                    this.toast.errorToastr(result['statusMsg']);
                }
        ...
        contractortList(searcValue) {
            this.service.post_rqst({'search':searcValue, 'point_transfer_right':'No', 'scanning_rights':'Yes'}, "Influencer/influencerAsPerScanningRights").subscribe((result => {
                if (result['statusCode'] == 200) {
                    this.contractorUser = result['result'];
                }
                else{
                    this.toast.errorToastr(result['statusMsg']);
                }
