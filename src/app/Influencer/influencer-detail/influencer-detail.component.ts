  ...
  InfluencerDetail(){
    ...
    this.service.post_rqst({'id':this.id , 'filter':this.filter },'Influencer/influencerCustomerDetail').subscribe((resp)=>
    {
      ...
      if (resp['statusCode'] == 200) {
        ...
        this.user_assign_detail = this.Influencer_Detail['user_assign_detail']
        console.log(this.user_assign_detail)
        for (let index = 0; index < this.user_assign_detail.length; index++) {
          this.user_assign_name += this.user_assign_detail[index];
          console.log(this.user_assign_name)
        }
        console.log(this.user_assign_name)
